...
public interface ProfileRepo extends Neo4jRepository<ProfileDetails,String> {
   ...
   @Query("MATCH(u:ProfileDetails {emailId:$email}) RETURN u")
    ...
    @Query(" MATCH (a:ProfileDetails {email:$email}),(b:location {name:$location}) MERGE (a)-[r:loc_of]->(b)")
   void createLocationRelationshipWithPersonalDetails(String email, String location);
    @Query(" MATCH (a:ProfileDetails {email:$email}),(b:skill {name:$skill}) MERGE (a)-[r:has_skl]->(b)")
    void createSkillRelationshipWithPersonalDetails(String email, String skill);
